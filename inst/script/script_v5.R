...
PLOT <- function(St=St, St00=St00, Dat05=Dat05, xlab=xlab, Background=Background, alpha=alpha, Symbols=Symbols){
...
if(E2 < 0){
rect(D[n]+2, A[1],
     D[n+1]-2, A[2], col=rgb(1,0,0, alpha=alpha),
     border=NA)
G <- c(G, "red1")
}else{
rect(D[n]+2, A[1],
     D[n+1]-2, A[2], col=rgb(0,1,0, alpha=alpha),
     border=NA)
G <- c(G, "green1")
}}}
...
PLOT(St=St, St00=St00, Dat05=Dat05, xlab=xlab, Background=Background, alpha=alpha, Symbols=Symbols)
grDevices::dev.off()
}else{
PLOT(St=St, St00=St00, Dat05=Dat05, xlab=xlab, Background=Background, alpha=alpha, Symbols=Symbols)
}
